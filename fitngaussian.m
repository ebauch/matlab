function results = fitngaussian(data, parameters)
% fit n Gaussian peaks to xydata with automated guessing of peak locations
% Erik Bauch, May 2017
...
% build n-peak Gaussian model function

% model = 'A1/(1 + ((x - center1)/(gamma1/2))^2) + offset'; % first Lorentzian

model = 'A1*exp(-1/2*((x-center1)/sigma1)^2) + offset'; % first Gaussian

for i = 2 : npeaks
   
    model = [model sprintf(' + A%d*exp(-1/2*((x-center%d)/sigma%d)^2)', i, i, i)];
    
end
           
            % intial parameters [amplitudes centers linewidths offset]                        
            sigmas = lw * ones(npeaks, 1);
            startpoints = [freqamp' freq' sigmas' offset];
